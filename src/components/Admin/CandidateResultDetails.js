...
export default class CandidateResultDetails extends Component {
  ...
  handleSubmit = () => {
    if (this.state.user.candidateStatus === 'SELECTED') {
      this.handleEventTrackForSelectCandidate()
    }
    else if (this.state.user.candidateStatus === 'REJECTED') {
      this.handleEventTrackForRejectingCandidate()
    }
    else if (this.state.user.candidateStatus === 'HOLD') {
      this.handleEventTrackForHoldCandidate()
    }
  ...
  handleEventTrackForSelectCandidate = () => {
    window.dataLayer.push({
      event: 'Select_Candidate'
    });
  }

  handleEventTrackForRejectingCandidate = () => {
    window.dataLayer.push({
      event: 'CandidateRejection'
    });
  }

  handleEventTrackForHoldCandidate = () => {
    window.dataLayer.push({
                  ...
                  <div>
                    <div style={{display:"flex"}}>
                      ...
                      <div onClick={this.state.profilePic ? this.reSizeImg : null} className="card-block">
                        <h6 className="m-b-20 p-b-5 b-b-default f-w-600" style={{ border: 'none', color: '#fc3f06' }}>ACADEMIC</h6>
                        <div className="row">
                        ...
                        {!this.state.totalResult.projectRound ? <h6 className="m-b-20 m-t-40 p-b-5 b-b-default f-w-600" style={{ border: 'none', color: '#fc3f06', marginBottom: '0px', marginTop: '50px' }}>ONLINE TEST SCORE</h6> : null}
                        <div className="row" style={{ paddingLeft: '15px', width: '46rem' }}>
                          {!this.state.totalResult.projectRound ?
                            <table className="table table-hover" style={{ opacity: '65%', marginBottom: '0px' }}>
                              <thead style={{ backgroundColor: '#E0E1EA' }}>
                                <tr style={{ textAlign: 'left' }}>
                                  <th className='thdesign' style={{ fontWeight: '2000', color: '#000000' }}>Section</th>
