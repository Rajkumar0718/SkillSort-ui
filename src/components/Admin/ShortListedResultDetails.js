...
const path = window.location.pathname.replace('/shortlisted-candidate-details/', '')
export default class ShortListedResultDetails extends Component {
          ...
          return _.map(result.results, (res) => {
            return (
              <tr className='rowdesign' style={{ paddingLeft: '15px' }}>
                ...
                <td style={{ fontWeight: '400', color: this.getColorsForSkill(res), fontSize: '13px', paddingLeft: '0px' }}><b>{this.calculateSkill(res)}</b></td>
              </tr>)
          })
        }
      })
  ...
  getProfilePic = () => {
    if (this.state.programmingRound[0]?.screenShot || this.state.sqlRound[0]?.screenShot) {
                ...
                <div style={{display:"flex"}}>
                  <div className="col-sm-4 user-profile">
                    ...
                    <div className='row' style={{ display: 'flex',float:'right' }}>
                      ...
                      <div className='card-block content-align' style={{ paddingTop: '0px' }}>
                        <button onClick={() => this.setState({ viewProfile: !this.state.viewProfile ,type:'certificate'})} className='btn btn-sm btn-nxt' style={{ fontFamily: 'Montserrat' }}>View Certficate</button>
                      </div> :null}
                    </div>
                  </div>
                  ...
                  <div className="card-block" onClick={this.state.profilePic ? this.reSizeImg : null}>
                    ...
                    <div className="row" style={{ paddingLeft: '15px', width: '46rem' }}>
                      <table className="table table-hover" style={{ opacity: '65%', marginBottom: '0px' }}>
                        <thead style={{ backgroundColor: '#E0E1EA' }}>
                          <th className='col-lg-3 thdesign' style={{ fontWeight: '2000', color: '#000000' }}>Section</th>
                          ...
                          <th className='col-lg-2 thdesign' style={{ fontWeight: '2000', color: '#000000' }}>Report</th>
                        </thead>
                        <tbody>
                          {this.renderTable()}
                        </tbody>
