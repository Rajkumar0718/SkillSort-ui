...
export default function AdvSearch(props) {
  ...
  const handleChange = (event, isClearAll) => {
    ...
    if (value[value.length - 1] === "all") {
      setDistricts(
        districts.length === States[states].length ? [] : States[states]
      );
      return;
    }
    setDistricts(value);
  ...
  const getSections = () => {
    axios
      .get(`${url.ADMIN_API}/section?isSkillSort=${true}`, {
        headers: authHeader(),
      })
      .then((res) => {
        let sections = _.filter(res.data.response, {
  ...
  const handleDepartmentChange = (event, isClearAll) => {
    ...
    if (value[value.length - 1] === "all") {
      setSelectedDepartment(
        selectedDepartment.length === departments.length
          ? []
          : _.map(departments, (dep) => dep.departmentName)
      );
      return;
    }
  ...
  const handleSubmit = (event) => {
    ...
    advSearch.districts =
      districts.length === 0 && states !== "" ? States[states] : districts;
  ...
  const getCandidates = (advSearch) => {
    axios
      .post(`${url.ADMIN_API}/adv-search`, advSearch, { headers: authHeader() })
      .then((res) => {
        ...
        <div style={{ padding: "10px", marginTop: "0.5rem" }}>
          <Card>
            <CardHeader
              title={
                <Typography
                  style={{ fontSize: "30px", fontFamily: "Baskervville" }}
                >
                  Advance search
                </Typography>
              }
                ...
                >
                  <div className="form-group row">
                    {fields.map((field, index) => (
                      <div
                        className="col-12 col-lg-6 col-sm-12 col-xl-6"
                        key={index}
                      >
                        <div className="row">
                      ...
                      <div className="row">
                        ...
                        <div className="col-9 col-sm-9 col-md-9 col-lg-9 col-xl-9  ">
                          <MultiSelectDropDown
                            ...
                            list={departments}
                            handleChange={(event, clear) =>
                              handleDepartmentChange(event, clear)
                            }
                            isObject={true}
                  ...
                  <div className="form-group row">
                    ...
                    {(isRoleValidation() === "ADMIN" ||
                      isRoleValidation() === "HR_MANAGAGER" ||
                      isRoleValidation() === "HR") && (
                      <div className="col-12 col-lg-6 col-sm-12 col-xl-6">
                        <div className="row">
                          <div className="col-3 col-sm-3 col-md-3 col-xl-3 col-lg-3">
                    ...
                    {isStudent && isRoleValidation() === "SUPER_ADMIN" ? (
                      <div className="col-12 col-lg-6 col-sm-12 col-xl-6">
                        <div className="row">
                          <div className="col-3 col-sm-3 col-md-3 col-xl-3 col-lg-3">
                            <label className="form-label input-label">
