  ...
  getStartButton = () => {
    ...
    if ((this.state.isOnlyProgramming && this.state.langSelected) || this.state.technology === 'PROGRAMMING') {
      if ((this.state.imgVisited)) {
        ...
        return (<div><Link className="btn btn-info disabled" style={{ position: 'relative', top: '50%', left: '50%', transform: 'translate(-45%,-125%)', padding: '3px 0px 0px 0px' }} to="/program" onClick={() => this.setLanguageInLocalStorage(this.state.language_id, this.state.languageName)}> {buttonText}</Link>
          <p style={{ fontSize: '15px', fontFamily: 'sans-serif',position:'relative', top:'-5px' }}><b>Watch all the images till the end to activate the start button</b></p></div>);
      }
  ...
  render() {
    return (
      <div className="candidate-main">
        <div className="can-instruction">Candidate Instruction{this.errorMsg.error}</div>
        ...
        <div style={{ display:'flex', justifyContent:'center'}}>{this.getCandidateInstruction()}</div>
      </div>
    );
  }
}
export default withLocation(CandidateInstructions)
